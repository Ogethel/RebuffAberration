...
namespace ALS.Aberration
{
    ...
    public class BootloadScreen : MonoBehaviour
    {
        ...
        [Tooltip("Show this warning message when in portrait mode")]
        ...
        private void Update()
        {
            // Update message only if the Screen size changes
            if (Screen.width != m_CurrentResolution.x || Screen.height != m_CurrentResolution.y)
            {
                ShowMessage();
            }
        }
...
//Documentation from the Quiz project from Unity:
